...
import { SortableBlogComponentForm } from "@/pages/blog/create/components/SortableBlogComponentForm";
...
export default function EditBlogPage() {
  ...
  useEffect(() => {
    if (blogData?.body) {
      ...
      if (blog.image) {
        setSelectedHeroImage(blog.image);
      }
    }
  }, [blogData, reset]);
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: async (data: BlogInput) => {
      const res = await instance.patch(`/blogs/${blogId}`, data, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  return (
    <div className="flex flex-col gap-4 w-full h-full text-white pb-10">
      ...
      <header className="flex items-center justify-between w-full">
        <h1 className="text-2xl font-semibold">Editar Blog</h1>
      </header>
