...
import type { ApiResponse, ClientResponse, ImageResponse } from "@/interface/response.interface";
...
export default function EditClientPage() {
  ...
  useEffect(() => {
    if (clientData?.body) {
      ...
      reset({
        name: client.name,
        url: client.url,
        feature: client.feature || false,
      });

      if (client.url) {
        setSelectedImage({ url: client.url } as ImageResponse);
      }
    }
  }, [clientData, reset]);
  ...
  return (
    <div className="flex flex-col gap-4 w-full h-full pb-10">
      <Load loading={isPending || isLoadingClient} />
